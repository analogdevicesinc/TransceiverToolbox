...
boardNames = [
    ...
    'pluto']
dockerConfig.add("-e HDLBRANCH=hdl_2023_r2")
...
for (int i=0; i < demoNames.size(); i++) {
    ...
    deployments[demo] = { node(nodeLabel) {
        stage("Demo Tests") {
            withEnv(['DEMO='+demo,'MLRELEASE=R2023b','HDLBRANCH=hdl_2023_r2','LC_ALL=C.UTF-8','LANG=C.UTF-8']) {
                try {
                    stage(demo) {
