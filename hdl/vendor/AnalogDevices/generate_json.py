...
ports_dict = {PROJECT_NAME: {'chip': ' ', 'complex': ' ', 'fpga': ' ', 'supported_rd': ' ', 'ports': ' '}}
    ...
    for line in fh:
        ...
        if key1 in line:
            ...
            if type1 in name_value:
              ...
              type_list.append(type1)  ## assign valid as data type for enable, sync and ovf signals
        elif key5 in line:
            net = line.split(' ')[0]
            if (number_of_words == 2):
    ...
    if ("os" not in name_list[i]):
      if (("pack" in name_list[i]) and ("_2_" not in name_list[i])):
        if (("fifo_wr_en" in name_list[i]) or ("fifo_wr_data" in name_list[i])):
          ...
          if (("adrv9371" in PROJECT_NAME) or ("adrv9009" in PROJECT_NAME) or ("adrv9001" in PROJECT_NAME)):
            dict_1 = {
                'input' : dir_list[i],
                'width' : msb_list[i] - lsb_list[i] + 1,
                'name' : name_list[i],
