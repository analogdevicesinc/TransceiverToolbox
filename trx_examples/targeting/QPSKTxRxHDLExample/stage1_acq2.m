% Acquisition protocol on the VQ build: golden live, held buffers, rstCS + trims.
% Also reads count_out to distinguish "processing but no preamble lock" from "deaf".
...
[y,~]=rx(); pause(3);
for trim=[0 2000 -2000 4000 -4000]
  BistRegisters.sshExec(sprintf('D=/sys/bus/iio/devices/iio:device2; echo %d > $D/out_altvoltage0_RX1_LO_frequency', 2400000000+trim),8); pause(1);
  ...
  fprintf('trim=%+5d: dcount=%-8d pkts/6s=%-6d errs=%d BER=%.4f%%\n', trim, c1-c0, p1-p0, e1-e0, 100*(e1-e0)/max(1,(p1-p0)*120));
end
