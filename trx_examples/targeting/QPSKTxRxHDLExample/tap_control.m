% Control: capture Receiver-input tap (debugI1/Q1 -> voltage1) in BOTH modes.
...
for mode={{'INTERNAL',0,0},{'CABLE',1,1}}
  nm=mode{1}{1}; wr('0x9D000114',mode{1}{2}); wr('0x9D000118',mode{1}{3}); pause(2);
  ...
  if isempty(z), fprintf('%s rx-input tap: EMPTY\n', nm); release(rx); continue; end
  zn=z/mean(abs(z)); rot=angle(mean(zn.^4))/4; zd=zn*exp(-1j*rot);
  ev=sqrt(mean(abs(zd/sqrt(2)-(sign(real(zd))+1j*sign(imag(zd)))/sqrt(2)).^2));
  fprintf('%s rx-input tap: N=%d rms=%.1f QI=%.3f circ=%.3f EVM=%.3f\n', nm, numel(z), rms(z), std(imag(z))/std(real(z)), abs(mean(z.^2))/mean(abs(z).^2), ev);
  release(rx);
end
release(tx);
