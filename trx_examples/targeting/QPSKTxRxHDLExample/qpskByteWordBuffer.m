function [word, avail, ready, state] = qpskByteWordBuffer(state, dataIn, validIn, popReq)
...
%   [word, avail, ready, state] = qpskByteWordBuffer(state, dataIn, validIn, popReq)
%
% word/avail expose the head entry; popReq && avail consumes it. ready is
% the value registered from the PREVIOUS step's occupancy (<2), matching
% the hardware where tready is a flop.
%
% Unit-tested single source of truth for the composite overlay's
% ByteWordBuffer MATLAB Function block.

if nargin == 0
    word = struct('buf', [uint64(0); uint64(0)], 'count', uint8(0), ...
