classdef HardwarePreDescramblerTest < matlab.unittest.TestCase
%HARDWAREPREDESCRAMBLERTEST  Read 1024 contiguous pre-Descrambler bits
%   captured by V18_predescrambler BOOT.BIN, run the LFSR descrambler in
%   MATLAB, and compute BER against the ADI Hello World reference.
%
%   Wave 1 hypothesis test: if MATLAB-descrambled BER << 7.3% post-Descrambler
%   BER, the Descrambler is amplifying errors (Wave 1 Branch A). Otherwise,
%   errors enter UPSTREAM of the Descrambler tap.
%
...
%   Output:   capture_word_out @ 0x9D00011C (32-bit word, 32 words = 1024 bits)
    ...
    methods (Test, TestTags = {'Hardware'})
        function decodePreDescrambler(testCase)
            ...
            % Unpack to 1024 bits (LSB-first per word, matching V18 BitCapture)
            bits = false(1,1024);
            for i = 0:1023
                w = floor(i/32) + 1;
                b = mod(i,32);
            ...
            % MATLAB self-synchronizing descrambler (poly + initial state from
            % the Tx Scrambler reference -- see QPSKRxChainTest.m)
            poly = [1 0 0 1 0 0 0];
            st   = [1 0 1 1 1 0 1];
            ...
            % Best alignment search over the 1024 descrambled bits.
            % Try each shift 0..119; the best should give near-zero errors
            % in the first 120-bit window (= first BIST window).
            ...
            % Per-packet error count (8 consecutive 120-bit windows)
            ...
            % BER computed across all matchable bits
            ...
            % Recover ASCII of first 120 bits for sanity
            pkt0 = aligned(1:120);
            ...
            fid = fopen(fullfile(outDir,'preDescrambler.json'),'w');
