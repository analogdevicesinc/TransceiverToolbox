%% probe_composite_rf3 - reboot + max Tx power + extended BER window
...
try, tx(complex(zeros(4096,1,'int16'), zeros(4096,1,'int16'))); catch ME, warning('%s', ME.message); end
fprintf('Tx armed @ 2.4 GHz atten=0 dB.\n');
...
pause(2);
fprintf('  tx.SamplingRate = %g, Atten readback = %g dB\n', tx.SamplingRate, tx.AttenuationChannel0);
fprintf('  rx.SamplingRate = %g\n', rx.SamplingRate);

%% Baseline @ rx_input_select=0
fprintf('\n--- baseline 5s @ rx_input_select=0 ---\n');
...
fprintf('  packets+=%d errors+=%d BER=%.3f%%\n', p1-p0, e1-e0, 100*(e1-e0)/max((p1-p0)*BitsPerPacket,1));

%% Flip to cable loopback, settle, measure 60s
fprintf('\n--- rx_input_select=1, settle 5s, measure 60s ---\n');
