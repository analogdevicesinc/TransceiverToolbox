% variant_pre_composite_capture_buf.m  --  composite_bit_capture
%
% V16-style 1024-bit capture buffer adapted to the composite topology.
% Adds a MATLAB Function (BitCapture) inside the Receiver that captures
% 1024 contiguous Rx-decoded bits starting at the first dstart pulse
% AFTER a 1M-cycle warm-up. Buffer is exposed as 32x32-bit words via
% AXI4-Lite x"11C".
%
% Selector: iq_debug_mux (5 LSBs, 0..31) -> selects which 32-bit word
% Output:   capture_word_out, AXI4-Lite x"11C"
% Buffer freezes after 1024 captured bits. Reboot to recapture.
...
if ~isempty(find_system(loop,'SearchDepth',1,'BlockType','Outport','Name','capture_word_out'))
    fprintf('variant_pre_composite_capture_buf: capture_word_out already present, skipping.\n');
    save_system(sys,[],'OverwriteIfChangedOnDisk',true);
    return;
end

if isempty(find_system(rx,'SearchDepth',1,'Name','BitCapture'))
    fprintf('Inserting BitCapture block (composite variant)...\n');
    bc = [rx '/BitCapture'];
    ...
    bcLines = { ...
        ...
        '%#codegen' ...
        '% Wait ~1M ticks for chain to settle, then capture next 1024 valid bits.' ...
        '% No dstart input -- caller cross-correlates the captured stream against' ...
        '% the expected "ADI Hello World" pattern to determine bit-alignment.' ...
    ...
    % --- Tap signals at Receiver level (where Capture Data Bits exists) ---
    cdb = [rx '/Capture Data Bits'];
    phCdb = get_param(cdb,'PortHandles');
    srcDataOut = get_param(get_param(phCdb.Inport(1),'Line'), 'SrcPortHandle');  % bit stream
    srcValid   = get_param(get_param(phCdb.Inport(4),'Line'), 'SrcPortHandle');  % valid (matches V16 inport index)
    srcSel     = get_param([rx '/iq_debug_mux'],'PortHandles').Outport(1);

    % Rate-bridge iq_debug_mux (composite boundary at 1/15.36 MHz) into the
    % descrambler rate so the MATLAB Function block has single-rate inputs.
    ...
    % --- Receiver-level outport + composite-level outport with AXI ---
    ...
    fprintf('variant_pre_composite_capture_buf: BitCapture wired to AXI x"11C".\n');
end
