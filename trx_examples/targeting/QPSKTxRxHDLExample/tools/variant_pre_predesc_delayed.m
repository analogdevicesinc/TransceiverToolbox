% V22_predesc_delayed: capture 1024 contiguous Rx bits at the QPSK
% Demodulator output (pre-Descrambler), DELAYED -- arm only after the
% 10000th dstart pulse (~330 ms past boot, well past cold-start).
%
% V22 vs V18: same outports, same wiring, same Descrambler Inport(1/2/4)
% tap. Only the BitCapture MATLAB Function differs -- adds a uint16
% pktCount that gates the armed transition.
%
% Host MATLAB descrambles in software and compares to the BIST reference.
% Answers: are pre-Descrambler bits clean in steady state, or have they
% already developed errors before the Descrambler sees them?
%
...
if ~isempty(find_system(loop,'SearchDepth',1,'BlockType','Outport','Name','capture_word_out'))
    fprintf('V22 already instrumented; skipping insertions.\n');
...
if isempty(find_system(rx,'SearchDepth',1,'Name','BitCapture'))
    ...
    fprintf('V22: predesc outport indices data=%d valid=%d start=%d\n', idx_data, idx_valid, idx_start);
    ...
    fprintf('V22_predesc_delayed insertion complete.\n');
end
