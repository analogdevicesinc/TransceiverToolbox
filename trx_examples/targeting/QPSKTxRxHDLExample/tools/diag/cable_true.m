% cable_true -- correct procedure: soft reset FIRST (clears regfile to
% internal), THEN write x118=1/x114=1 (early-stream switch, proven in E2/E3).
% Measure BER with an in-run host-stop discriminator to certify cable.
...
% CORRECT ORDER: reset (clears to internal), then select host+cable
...
dp=p1-p0; de=e1-e0;
fprintf('CT CABLE-TRUE: pkts/30s=%d errors=%d BER=%.5f%% (errs/pkt=%.3f)\n', dp, de, 100*de/max(1,dp*120), de/max(1,dp));
% discriminator: stop host mid-state
release(tx); pause(2);
p0=rd('0x9D000104'); pause(8); p1=rd('0x9D000104');
fprintf('CT host stopped: pkts/8s=%d (must be ~0 if truly cable)\n', p1-p0);
release(rx);
fprintf('CABLE_TRUE_DONE\n');
