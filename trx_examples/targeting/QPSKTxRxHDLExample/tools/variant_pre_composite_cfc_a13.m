% cfc_a13: shorten the Coarse Frequency Estimator averaging window to 2^13
...
mi = strrep(mi,'integAvgLen = 2^15;','integAvgLen = 2^13;');
set_param(qrx,'MaskInitialization',mi);
assert(contains(get_param(qrx,'MaskInitialization'),'integAvgLen = 2^13;'),'overlay failed');
fprintf('cfc_a13: Coarse Frequency Estimator integAvgLen 2^15 -> 2^13 (%d sym)\n', 2^13);
