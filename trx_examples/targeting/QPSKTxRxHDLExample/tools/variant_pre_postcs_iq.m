% V17_postcs_iq: capture 64 complex symbols at the Carrier Synchronizer
...
if ~isempty(find_system(loop,'SearchDepth',1,'BlockType','Outport','Name','capture_word_out'))
    fprintf('V17 already instrumented; skipping insertions.\n');
...
if isempty(find_system(rx,'SearchDepth',1,'Name','PostCSCapture'))
    ...
    fprintf('V17: postcs outport indices I=%d Q=%d V=%d\n', idx_I, idx_Q, idx_V);
    ...
    pccLines = { ...
        ...
        '%#codegen' ...
        '% Capture 64 packed-IQ samples after first dstart, freeze.' ...
    ...
    % dstart source: use the existing Capture Data Bits dataSrt source
    % (post-Descrambler start; same 2-tick latency as V16, fine for capture trigger)
    ...
    add_line(rx, srcDataSrt,                            get_param(pcc,'PortHandles').Inport(4), 'autorouting','on');
    ...
    fprintf('V17_postcs_iq insertion complete.\n');
end
