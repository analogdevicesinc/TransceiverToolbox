% V23_postdesc_delayed: capture 1024 contiguous Rx-decoded bits at the
% POST-Descrambler dataOut (V16's tap point), DELAYED -- arm only after
% the 10000th dstart pulse (~330 ms past boot, well past cold-start).
%
% V23 vs V16: same wiring (Capture Data Bits inputs -> post-Descrambler
% dataOut/dataSrt/validOut), only the BitCapture MATLAB Function differs --
% adds a uint16 pktCount that gates the armed transition (same as V22).
%
% Diagnostic question: V22 (pre-Descrambler delayed) shows 44% BER and
% deterministic-garbage. If V23 (post-Descrambler delayed, same packet)
% yields the BIST data correctly (~2-12% V15-style ramp), then the HDL
% Descrambler has drifted from MATLAB's emulated descrambler -- its
% internal LFSR state diverged in a way the self-syncing property
% does not recover from. If V23 ALSO yields garbage, the divergence is
% upstream of the Descrambler (at QPSK Demod or Carrier Sync).
%
...
if ~isempty(find_system(loop,'SearchDepth',1,'BlockType','Outport','Name','capture_word_out'))
    fprintf('V23 already instrumented; skipping insertions.\n');
...
if isempty(find_system(rx,'SearchDepth',1,'Name','BitCapture'))
    ...
    bcLines = { ...
        ...
        '% count dstart pulses; arm on the 10000th (~330 ms past boot at' ...
        '% 30 kpkt/s); then freeze. pktCount is uint16 (max 65535) and is' ...
        '% incremented only while armed==0 -- it never overflows.' ...
    ...
    fprintf('V23_postdesc_delayed insertion complete.\n');
end
