% variant_pre_composite_fracdelay_ma3 -- insert a half-sample delay / smoothing FIR
% [1 1 1]/3 on the INTERNAL-loopback branch only (REP_Tx -> MUX inputs 3), to
...
if ~isempty(find_system(sys,'SearchDepth',inf,'Name','FRAC3_I'))
  fprintf('variant_pre_composite_fracdelay_ma3: already instrumented, skipping.\n'); return;
end

% Insert [1 1 1]/3 FIR on REP_TxI/Q -> MUX_RxI/Q input 3 (the internal-loopback path).
pairs = {'REP_TxI','MUX_RxI','FRAC3_I'; 'REP_TxQ','MUX_RxQ','FRAC3_Q'};
for k=1:size(pairs,1)
  ...
  delete_line(l);
  % add Discrete FIR Filter [1 1 1]/3 (full precision) + Data Type Conversion -> int16
  add_block('simulink/Discrete/Discrete FIR Filter', fracblk, ...
     'Coefficients','[1 1 1]/3', 'Position',[505 215+40*(k-1) 535 245+40*(k-1)]);
  convblk=[loop '/' frac 'c'];
  ...
  fprintf('inserted %s [1 1 1]/3 FIR + int16 conv on %s -> %s input 3\n', frac, src, mux);
end

save_system(sys,[],'OverwriteIfChangedOnDisk',true);
fprintf('variant_pre_composite_fracdelay_ma3: done (internal-loopback fractional delay).\n');
