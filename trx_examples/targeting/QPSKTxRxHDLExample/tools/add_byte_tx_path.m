...
%   Transmitter inports  5..7: extWord (uint64), extWordAvail (boolean),
%                              extBitSel (boolean); outport 9: extWordPop.
%   Input Data inports   3..5: same trio; outport 4: extWordPop.
...
idin = { 'extWord','uint64',3; 'extWordAvail','boolean',4; 'extBitSel','boolean',5 };
for k = 1:size(idin,1)
    blk = [id '/' idin{k,1}];
...
ch.Script = sprintf([ ...
'function [bit, popToggle] = byteBitShifterBlk(enable, reset, start, extWord, extWordAvail)\n' ...
...
'%% exported as a toggle (one flip per word latch) to survive the\n' ...
'%% slow->fast rate crossing; upstream edge-detects it back into a pulse.\n' ...
...
'    uint64(extWord), logical(extWordAvail));\n' ...
'if pop\n' ...
'    tog = ~tog;\n' ...
'end\n' ...
'popToggle = tog;\n']);
...
txin = { 'extWord','uint64',5; 'extWordAvail','boolean',6; 'extBitSel','boolean',7 };
for k = 1:size(txin,1)
    blk = [tx '/' txin{k,1}];
