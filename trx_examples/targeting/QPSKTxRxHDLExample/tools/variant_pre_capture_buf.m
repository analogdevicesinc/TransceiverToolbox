% V16_capture_buf: V3 baseline + raw bit-stream capture buffer inside
% Receiver. Captures 1024 contiguous Rx-decoded bits (starting at first
% dstart after reset, ~8.5 packets) into an in-FPGA buffer; the buffer
% is exposed as 32x32-bit words via AXI4-Lite x"11C". The host MATLAB
% reads it back word-by-word and reconstructs the actual bit sequence
% that the receiver decoded -- revealing exact error positions, sync
% slips, burst patterns, etc.
%
% Selector: iq_debug_mux (5 LSBs, 0..31) -> selects which 32-bit word
% Output:   capture_word_out, AXI4-Lite x"11C"
%
% Buffer freezes after 1024 captured bits. Reboot the board to recapture.
...
if isempty(find_system(rx,'SearchDepth',1,'Name','BitCapture'))
    ...
    bcLines = { ...
        ...
        '%#codegen' ...
        '% Capture 1024 bits of Rx output bit stream into a 32-word x 32-bit buffer.' ...
        '% Capture starts on first dstart, freezes after 1024 bits captured.' ...
        '% addr_sel (0..31) selects which 32-bit word to expose.' ...
    ...
    fprintf('V16_capture_buf insertion complete.\n');
end
