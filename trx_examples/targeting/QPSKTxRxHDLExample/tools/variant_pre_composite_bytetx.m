...
out_byte_rx = { 'byte_rx_data','13'; 'byte_rx_valid','14'; 'byte_rx_last','15'; 'byte_rx_user','16' };
...
wppNum = commhdlQPSKTxRxParameters().DataBitsPerPacket / 64;
...
'word = heldWord; wordTog = tog; wordLast = heldLast; wordFirst = heldFirst;\n']), ...
    'WORDS_PER_PACKET', num2str(wppNum));
...
% --- (3) patch the kit hdlworkflow_loopback.m: IOInterface mappings + RD ---
% Single idempotent pass built by a loop (no sprintf placeholder counting):
% every (port, interface, mapping) tuple for the verif, byte-TX, and byte-RX
% paths, inserted once after the bit_errors_out anchor; the workflow's
% ReferenceDesign is retargeted at the byte variant, whose exact name is
% read from the plugin so it can never drift from hdl/vendor.
...
if ~contains(wfTxt, 'composite-byte mappings')
    maps = { ...
      % verif host/debug paths
      'host_txI',       'IP Data 0 IN [0:15]',   '[0:15]'; ...
      'host_txQ',       'IP Data 1 IN [0:15]',   '[0:15]'; ...
      'host_txValid',   'IP Valid Tx Data IN',   '[0]'; ...
      'tx_source_select','AXI4-Lite',            'x"118"'; ...
      'debugI',         'IP Data 2 OUT [0:15]',  '[0:15]'; ...
      'debugQ',         'IP Data 3 OUT [0:15]',  '[0:15]'; ...
      'debugValid',     'IP Data Valid OUT',     '[0]'; ...
      'debugI1',        'IP Data 0 OUT [0:15]',  '[0:15]'; ...
      'debugQ1',        'IP Data 1 OUT [0:15]',  '[0:15]'; ...
      % byte-TX path
      'byte_data',      'Byte Data IN [0:63]',   '[0:63]'; ...
      'byte_valid',     'Byte Valid IN',         '[0]'; ...
      'byte_ready',     'Byte Ready OUT',        '[0]'; ...
      'byte_first',     'Byte First IN',         '[0]'; ...
      'tx_data_source', 'AXI4-Lite',             'x"11C"'; ...
      % byte-RX path
    ...
    L = ['% --- composite-byte mappings ---' newline];
    for k = 1:size(maps,1)
