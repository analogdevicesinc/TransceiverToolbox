...
%% transmit waveform
scale = 28000; % less than 2^15-1 to account for some scale growth by Farrow filter in the IP
...
hw_tests.corr_thresh = 150; % obtained by first running find_corr_threshold.m
hw_tests.debug_mode = 0;
hw_tests.buflen = 2^15;
hw_tests.kbc = ceil(length(tx_wf)/hw_tests.buflen)+1;
hw_tests.rx_gain = 30;
hw_tests.tx_attn = -5;
...
%% read estimated scale and fractional offset (0 <= fd_hat <= 15) from registers 
fd_hat = hw_tests.w6();
adjusted_scaleI = hw_tests.w7();
adjusted_scaleQ = hw_tests.w8();
mag_scale = abs(double(adjusted_scaleI)+1i*double(adjusted_scaleQ));
angle_scale = rad2deg(angle(double(adjusted_scaleI)+1i*double(adjusted_scaleQ)));

%% plot time-series data
figure; 
subplot(2,1,1);
plot(real(hw_tests.txIQ(1:5e4))); 
hold on;
plot(real(hw_tests.rxIQ(1:5e4))); 
legend('Tx (I)', 'Rx (I)');
xlabel('sample index');
title('the first 50000 samples of input and output waveforms');
subplot(2,1,2); 
plot(imag(hw_tests.txIQ(1:5e4))); 
hold on;
plot(imag(hw_tests.rxIQ(1:5e4))); 
legend('Tx (Q)', 'Rx (Q)');
xlabel('sample index');
...
figure; 
subplot(2,1,1);
plot(real(hw_tests.txIQ(4e4+1:4e4+200))); 
hold on;
grid on;
plot(real(hw_tests.rxIQ(4e4+1:4e4+200))); 
legend('Tx (I)', 'Rx (I)');
xlabel('sample index');
title('an arbitrary window of 200 samples of input and output waveforms');
subplot(2,1,2); 
plot(imag(hw_tests.txIQ(4e4+1:4e4+200))); 
hold on;
grid on;
plot(imag(hw_tests.rxIQ(4e4+1:4e4+200))); 
legend('Tx (Q)', 'Rx (Q)');
xlabel('sample index');
...
function hwt = configure_ip(hwt)
    ...
    % estimated fractional delay
    w6 = matlabshared.libiio.aximm.read('uri','ip:192.168.3.2');
    w6.AddressOffset = hex2dec('120');
    w6.HardwareDataType='int16';
    
    % scale factor (I)
    w7 = matlabshared.libiio.aximm.read('uri','ip:192.168.3.2');
    w7.AddressOffset = hex2dec('11C');
    w7.HardwareDataType='int16';
    
    % scale factor (Q)
    w8 = matlabshared.libiio.aximm.read('uri','ip:192.168.3.2');
    w8.AddressOffset = hex2dec('118');
    w8.HardwareDataType='int16';
...
function hwt = send_waveform(hwt, rx, tx)
    ...
    count = 1;
    txIQ = zeros(hwt.buflen, hwt.kbc);
    rxIQ = zeros(hwt.buflen, hwt.kbc);
    ...
    % send transmit waveform through DMA and capture receive waveform
    tx(hwt.tx_wf);
    valid = 1;
    while (valid)
        [out, valid] = rx();
