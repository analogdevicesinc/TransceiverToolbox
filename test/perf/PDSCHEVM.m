...
%     updateEVMPlots();
    ...
    if ~skip
        frameEVM = [frameEVM; lteEVM(cat(1, evm(1).EV)) ]; %#ok<AGROW>
        frameEVM = [frameEVM; lteEVM(cat(1, evm(2).EV)) ]; %#ok<AGROW>
    end
    
%     % After we've filled a frame or if we're at the end of a signal
%     % shorter than a frame, do EVM averaging
%     if ~skip && (mod(i, 10)==9 || (nFrames==0 && i==nSubframes-1))
%         if (nFrames==0)
...
if ~isempty(frameEVM)
    ...
    error('No packets found for decode');
%     finalEVM = 0;
end
