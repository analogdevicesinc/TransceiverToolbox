function runHWEmulatedTests(board)
...
param = Parameter.fromData('EmulationEnabled',EmulationEnabled);

if nargin == 0
    suite = testsuite(ats);
else
    % suite = testsuite(ats,'ExternalParameters',param);
    ...
    suites = {};
    for i = 1:length(ats)
        mc = metaclass(eval(ats{i}));
        suites{i} = TestSuite.fromClass(mc,'ExternalParameters',param);
    end
    % Merge all suites into one
    suite = [suites{:}];
    suite = selectIf(suite,HasProcedureName(ContainsSubstring(at,'IgnoringCase',true)));
end
